...
const Sidebar = ({isSidebarOpen, toggleSidebar}) => {
    ...
    <aside
       className={`fixed top-0 left-0 h-full w-64 bg-white/90 backdrop-blur-lg border-r border-slate-200/60 z-50 md:relative md:w-64 md:shadow-0 md:flex md:flex-col md:translate-x-0 transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
    ...
    <div className="flex items-center justify-between h-16 px-5 border-b border-slate-200/60">
        ...
        <button onClick={toggleSidebar} className="md:hidden text-slate-500 hover:text">
            <X size={24} />
        </button>
    </div>
